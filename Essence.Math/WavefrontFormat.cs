...
namespace Essence.Maths
{
    public class WavefrontFormat : DisposableObject
    {
        ...
        public int AddTexture(Vec2d tex)
        {
            this.WritePadding();
            this.streamWriter.WriteLine(string.Format(en_US,
                                                      "vt {0:F3} {1:F3}",
        ...
        public void AddPoints(IEnumerable<Vec3d> points)
        {
            List<int> indices = points.Select(p => this.AddVertex(p)).ToList();
            this.AddPoints(indices);
        }

        public void AddPoints(IEnumerable<Vec2d> points)
        {
            List<int> indices = points.Select(p => this.AddVertex(p)).ToList();
            this.AddPoints(indices);
        }
        ...
        public void AddLines(IEnumerable<Vec3d> points, bool close = false)
        {
            List<int> indices = points.Select(p => this.AddVertex(p)).ToList();
            this.AddLines(indices, close);
        }

        public void AddLines(IEnumerable<Vec2d> points, bool close = false)
        {
            List<int> indices = points.Select(p => this.AddVertex(p)).ToList();
            this.AddLines(indices, close);
        }
        ...
        public void AddFace(IEnumerable<Vec3d> points)
        {
            List<int> indices = points.Select(p => this.AddVertex(p)).ToList();
            this.AddFace(indices);
        }

        public void AddFace(IEnumerable<Vec2d> points)
        {
            List<int> indices = points.Select(p => this.AddVertex(p)).ToList();
            this.AddFace(indices);
        }
    ...
    public class MaterialFormat : DisposableObject
    {
        ...
        public class Mat
        {
            ...
            public Mat(string name, Vec3d diffuse)
            {
                this.Name = name;
                this.Ambient = null;
